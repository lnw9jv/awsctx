...
- **`internal/aws/`** — Reads `~/.aws/config` (or `$AWS_CONFIG_FILE`) with a single `bufio.Scanner` pass (no external INI library). Exposes two functions: `LoadProfiles` (returns `[]string` names) and `LoadProfileDetails` (returns `[]Profile` with `Name` and `AccountID`). The scanner tracks the current section header: `[default]` → `Profile{Name:"default"}`, `[profile foo]` → `Profile{Name:"foo"}` (the `profile ` prefix is stripped); any other section (e.g. `[sso-session ...]`) is ignored. `AccountID` resolves via `accountID` with three fallbacks in order: `sso_account_id` key → `account_id` key → account segment parsed from `role_arn` (e.g. `arn:aws:iam::123456789012:role/Name`). Only full-line `#`/`;` comments are skipped; trailing inline comments are kept as part of the value (matching the prior `IgnoreInlineComment` behavior).

- **`internal/state/`** — Manages the "previous profile" as a plain file at `~/.cache/awsctx/previous` (overridable via `AWSCTX_STATE_DIR`). Two package-level functions, both taking the state dir: `SetPrevious(dir, profile)` and `GetPrevious(dir)`.
